...
function EditorRegister() {
	...
	const handleSubmit = async (e) => {
		...
		try {
			await axios.post(
				`${import.meta.env.VITE_BACKEND_URL}/api/auth/editor/register`,
				formData
			);
	...
	return (
		<div className="flex items-center justify-center min-h-screen bg-[#f8fafc] p-4">
			<form
				...
				className="mt-15 bg-white p-8 rounded-xl shadow-md w-full max-w-lg space-y-6 border border-[#e2e8f0]"
			>
				...
				<div>
					...
					<select
						...
						className="w-full px-4 py-2 rounded-lg bg-[#f8fafc] text-[#1a365d] border border-[#e2e8f0] focus:border-[#496580] focus:ring-2 focus:ring-[#496580]/30 outline-none"
					>
